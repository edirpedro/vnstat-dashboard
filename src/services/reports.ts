...
class Reports {
  ...
  private _json: IvnStat.Interface;
  private _unit: IvnStat.Unit;

  /**
   * Instantiate the object to manipulate reports.
   * @param {array} json - Traffic data retrieved from vnStat
   * @returns {instance}
   */
  constructor(json: IvnStat.Interface, unit: IvnStat.Unit) {
    this._json = json;
    this._unit = unit;
    this.prepare();
    return this;
  ...
  getTotal(): IvnStat.Total {
    return this._json.traffic.total;
  }
  ...
  getTraffic(type: IvnStat.TrafficKeys, gaps: boolean = false): IvnStat.Traffic[] {
    let traffic = this._json.traffic;
    let data: IvnStat.Traffic[] = [];
    data = this.clone(traffic[type]) as IvnStat.Traffic[];
  ...
  getAllTraffic(gaps: boolean = false): IvnStat.Traffics {
    let traffic = this.clone(this._json.traffic) as IvnStat.Traffics;
